...
%     healthy = [healthy models_n([1 2 4 5 24 25 26 27 100],(MS_n>MSinterval(i,1) & MS_n<MSinterval(i,2)-1800000*(i~=length(MSbradyPos))))];
%     risk = [risk models_n([1 2 4 5 24 25 26 27 100],(MS_n>=MSbradyPos(i)-1800000 & MS_n<MSbradyPos(i)))];
% end
...
healthy_in=[]; risk_in=[];
for i = 1:length(MSbradyPos)
    healthy_in = [healthy_in MS_n(MSpos>MSinterval(i,1) & MSpos<MSinterval(i,2)-1800000*(i~=length(MSbradyPos)))];
    risk_in = [risk_in MS_n(MSpos>=MSbradyPos(i)-1800000 & MSpos<MSbradyPos(i))];
end

% clearvars -except healthy risk
% 
% %------------------------------test classification------------------------
% h = randperm(length(healthy),100);
% ht = randperm(100,10);
% h_te = h(ht); %indices of testing set :healthy
% h_tr = h; h_tr(ht) = []; %indices of training set: healthy
% 
% r = randperm(length(risk),100);
% rt = randperm(100,10);
